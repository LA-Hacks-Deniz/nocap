...
- [x] **@devin — 2026-04-25 19:50 (started) → 2026-04-25 21:30 (done)**
- **Deliverable**: (1) `spec.py` `_JSON_INSTRUCTION` gains an "Equation ranking — REQUIRED (function-aware runs)" section that tells the function-aware Spec to rank `claimed_equations` in descending priority order — function-defining equations (LHS = function output) FIRST, intermediate assignments next, notational definitions last — and to OMIT pure notational-definition equations (e.g. `\bar{\alpha}_t = \prod \alpha_s`, `\hat{m}_t \equiv m_t / (1-\beta_1^t)`) from `claimed_equations[]` entirely on function-aware runs (they describe notation, not behavior, so comparing them to the function's return value is structurally meaningless). Updates the Adam worked example to lead with `\theta_t = ...` and adds a new transformer-attention worked example showing `Attention(Q,K,V) = softmax(QK^T/sqrt(d_k)) V` correctly leading the equation list (vs the wrong version that drops `sqrt(d_k)` by extracting only the intermediate `scores = QK^T`). (2) `orchestrator._strategy_evidence` symbolic/numerical iteration loop: GUARDRAIL on the existing T1.22-era `_heuristic_target_var` `_return` fallback — for `i > 0` (non-leading equations), force-skip when the target resolved to `_return` because the helper's bridge is only valid for the function-defining equation at `i = 0`; for non-leading intermediates / notational-defs the comparison is structurally bogus and produces confidently-wrong `equivalent=False` from the matcher and judge. (3) `skipped_equations` widened from 2-tuple `(index, target)` to 3-tuple `(index, target, reason)` so the JSONL stream + downstream debugging can distinguish *why* each equation was dropped (reasons: `non-leading_return_fallback`, `target_absent`, `matcher_var_not_found`).
